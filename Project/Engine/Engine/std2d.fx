...
float4 PS_Std2D(VTX_OUT _in) : SV_Target
{
    ...
    if (g_btex_1)
    {
        ...
        vNormal = mul(float4(vNormal, 1.f), g_matWorld).xyz;        
        vNormal.z = 0.f;        
    }     
    
    float3 vLightColor = CalculateLight2D(_in.vWorldPos, vNormal);
    
    vOutColor.rgb *= vLightColor.rgb + float3(0.15f, 0.15f, 0.15f);
