...
void CEventMgr::update()
{
	...
	for (size_t i = 0; i < m_vecEvent.size(); ++i)
	{
		...
		case EVENT_TYPE::CHANGE_AI_STATE:
		{
			...
			const std::wstring NextStateName = (const wchar_t*)m_vecEvent[i].wParam;
			//wstring NextStateName = (const wchar_t*)m_vecEvent[i].wParam;


			const auto& iter = pFSM->m_mapState.find(NextStateName);

			if (nullptr == iter->second)
				break;

			if (nullptr == pFSM->m_pCurState)
			{
				pFSM->m_pCurState = iter->second;
				return;
			}

			if (nullptr != pFSM->m_pCurState->m_ExitFunction)
			{
				pFSM->m_pCurState->m_ExitFunction();
			}

			pFSM->m_pCurState = iter->second;

			if (nullptr != pFSM->m_pCurState->m_EnterFunction)
			{
				pFSM->m_pCurState->m_EnterFunction();
			}
