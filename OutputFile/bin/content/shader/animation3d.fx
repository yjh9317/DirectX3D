...
// Scale은 1로 고정한 버전
void _MatrixAffineTransformation(in float4 RotationOrigin
    , in float4 RotationQuaternion
    , in float4 Translation
    , out matrix _outMat)
{
...
RWStructuredBuffer<matrix> Socket : register(u1);
RWStructuredBuffer<matrix> Socket2 : register(u2);

// ===========================
// Animation3D Compute Shader
#define BoneCount   g_int_0
#define CurFrame    g_int_1
#define IsAnim      g_int_3


#define Ratio       g_float_0
#define PrevFrame   g_float_1
#define BoneIndex   g_float_2
#define BoneIndex2  g_float_3
// ===========================
[numthreads(256, 1, 1)]
void CS_Animation3D(int3 _iThreadIdx : SV_DispatchThreadID)
{
    ...
    uint iPrevFrame = BoneCount * PrevFrame + _iThreadIdx.x;
    ...
    if (IsAnim == 1)
    {
        vScale = lerp(g_arrFrameTrans[iPrevFrame].vScale, g_arrFrameTrans[iFrameDataIndex].vScale, Ratio);
        vTrans = lerp(g_arrFrameTrans[iPrevFrame].vTranslate, g_arrFrameTrans[iFrameDataIndex].vTranslate, Ratio);
        qRot = QuternionLerp(g_arrFrameTrans[iPrevFrame].qRot, g_arrFrameTrans[iFrameDataIndex].qRot, Ratio);
    }
    else
    {
    ...
    // 구조화버퍼에 결과값 저장 , matOffset은 정점 위치 제거용 행렬
    g_arrFinelMat[_iThreadIdx.x] = mul(matOffset, matBone);

    // BoneIndex에 해당하는 뼈의 Transform을 저장
    if (_iThreadIdx.x == BoneIndex)
    {
        matrix Tmp = (matrix)0.f;
        _MatrixAffineTransformation(vQZero, qRot, vTrans, Tmp);
        Socket[0] = Tmp;
    }

    if (_iThreadIdx.x == BoneIndex2)
    {
        matrix Tmp = (matrix)0.f;
        _MatrixAffineTransformation(vQZero, qRot, vTrans, Tmp);
        Socket2[0] = Tmp;
